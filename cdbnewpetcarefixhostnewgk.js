  ...
  //-----------------------------------------------------------------------------------//
    ...
    //----------------------------------------------------------encrypt---------------------------------------------------//
        ...
        function encryptcodedelete(){
            ...
            var encryptcodedelete = CryptoJS.AES.encrypt(codedelete, "Caredible");
    ...
    //----------------------------------------------------------encrypt---------------------------------------------------//
                ...
                if (snapshot.exists()) {
                    snapshot.forEach((childSnapshot) => {
                        ...
                        if (inputcekreservasisszz == jawabanSgnuPemgm) {
                            ...
                            var inputannamapasien = childSnapshot.val().jawabannamapasien;
                            var decryptednamapasien = CryptoJS.AES.decrypt(inputannamapasien, "Caredible");
                            var inputanjeniskelamin = childSnapshot.val().jawabantanggallahir;
                            var decryptedjeniskelamin = CryptoJS.AES.decrypt(inputanjeniskelamin, "Caredible");
                            var inputankodepin = childSnapshot.val().kodepin;
                            var decryptedkodepin = CryptoJS.AES.decrypt(inputankodepin, "Caredible");
        ...
        $(document).ready(function(){
            ...
            get(dbRefVerifikasi).then((snapshot) => {
                if (snapshot.exists()) {
                    snapshot.forEach((childSnapshot) => {
                        ...
                        var decryptedcodeverifikasi = CryptoJS.AES.decrypt(codeverifikasizz, "Caredible");
                        var jawabanSgnuPemgmzz = decryptedcodeverifikasi.toString(CryptoJS.enc.Utf8);
                        var ubahusernamekeep = "caredibleverifikasidentitas";
            ...
            function showNotificationVerfikasi() {
        ...
        $("#izinkan").on("click",function(){
            var fontstyle = document.getElementById('fontstyle').value;
            let code_valverifikasi = Math.floor(1000 + Math.random() * 9000);
            $("#izinkan").hide();
            $("#bloknamaverifikasi1").text('klik tombol dibawah untuk mendapatakan kode verifikasi');
            $("#blokVerifikasi").append('<button type="button" id="kirimkode" style="font-family: ' + fontstyle + ';">request kode verifikasi</button>');
            $("#kirimkode").css('font-family', fontstyle.value );
            $("#kirimkode").click(function() {
                function pushnotifkode() {
                    if ('serviceWorker' in navigator) {
                        navigator.serviceWorker.ready.then((registration) => {
                            registration.showNotification('Kode Verifikasi', {
                                body: code_valverifikasi,
                                icon: 'https://lh3.googleusercontent.com/pw/ADCreHeMyQNLrlCLBwq2ZjmyQWPN3PcPRTs2BYwAd95bSNoA8XKeTH1UXhthvd1rwsBnkoGLnMyehKpe0SjFku3EEhERwA-mvAXJKpnulZVq3UdG_KDawYI=w2400',
                                badge: 'https://lh3.googleusercontent.com/pw/ADCreHf-YBEYd1qUrmGBkV5inxbzXHiCaFLT1YQ_0ssGTMpJkFsYGeUyZ_iyoCc-C1C7_qc01lNFM8m0MekdSj3cEp8Iywi5C2W7Nk_1y79VLu6MI6mBhgk=s41-p-k'
                            });
                        });
                ...
                pushnotifkode();
                $("#kirimkode").hide();
                $("#bloknamaverifikasi1").text('silahkan input kode verifikasi yang tertera di-push notifikasi');
                var fontstyle = document.getElementById('fontstyle').value;
                $("#blokVerifikasi").append('<input type="text" id="confirmkode" placeholder="input kode verifikasi" style="font-family: ' + fontstyle + ';">');
                $("#confirmkode").on("change",function(){
                    var confirmkode = $("#confirmkode").val();
                    if (confirmkode == code_valverifikasi) {
                        $("#confirmkode").css('border', '2px solid green');
                        localStorage.setItem('caredibleverifikasidentitas', code_valverifikasi);
                        verifikasiPasien();
                        $('#confirmkode').css('background-image', 'url("https://lh3.googleusercontent.com/pw/ADCreHecSdkRRvo0Z3kFO6fVK6Ex-ZvHMVziKMeIRfsLW9zhwIrDeswdXIEbnnfEGFcWVpyRSXIUi7NtZFjeXFB1xMuxjXDRwLf6wC2siOxzz2dPT5cxbMo=w2400")');
                        $('#confirmkode').css('background-size', '9%');
        ...
        function encryptverifikasiPasien(){
            var codeverifikasi = $("#confirmkode").val();
            var encryptedverifikasiPasien = CryptoJS.AES.encrypt(codeverifikasi, "Caredible");
    ...
    //----------------------------------------------------------encrypt---------------------------------------------------//
        ...
        function databasereservasiHealth() {
            onChildAdded(ref(database, databasereservasi+"/Health/"), (snapshot) => {
                ...
                var decryptedcodedelete = CryptoJS.AES.decrypt(inputancodedelete, "Caredible");
        ...
        function databasereservasiDone() {
            onChildAdded(ref(database, databasereservasi+"/Done/"), (snapshot) => {
                ...
                var decryptedcodedeletedone = CryptoJS.AES.decrypt(inputcodedeletedone, "Caredible");
        ...
        onChildAdded(ref(database, databasethemez), (snapshot) => {
            ...
            $("#izinkan").css('font-family', fontstyle.value );
